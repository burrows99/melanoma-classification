...
\textbf{Exp.~1}~($+$cosine annealing), \textbf{Exp.~2}~(switches to AdamW, which couples weight decay
$10^{-5}$ with the optimiser), \textbf{Exp.~3}~($+$reduced $\gamma{=}1.5$), and
\textbf{Exp.~4}~(image-only ablation, same HP as Baseline but metadata
branch disabled). Exp.~1--3 each introduce
\emph{one primary change} over their predecessor (Exp.~2 couples optimiser
and weight decay as AdamW inherently includes decoupled WD), enabling
controlled isolation of each factor's effect. The model checkpoint with the highest

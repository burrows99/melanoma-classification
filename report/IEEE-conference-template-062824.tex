...
%
\textbf{Baseline} — Adam, fixed LR, $\gamma{=}2.0$, with metadata; a
minimal-tricks reference.
%
\textbf{Exp.~1} adds cosine-annealing LR scheduling to help the model settle
into sharper minima and improve F1 without extra FP.
%
\textbf{Exp.~2} replaces Adam with AdamW (decoupled weight decay $10^{-5}$)
to reduce overfitting on the minority class and boost recall.
%
\textbf{Exp.~3} lowers $\gamma$ from 2.0 to 1.5, down-weighting easy
negatives less aggressively to recover specificity lost from the broader
decision boundary in Exp.~2.
%
\textbf{Exp.~4} disables the metadata MLP (same HP as Baseline) to quantify
the exact contribution of metadata fusion.
%
...
Notably, TTA trades 0.7~pp recall (92.16\%$\,{\rightarrow}\,$91.48\%) for
55 fewer false positives, improving F1 and specificity at a small sensitivity
cost---an acceptable trade-off for a screening tool where FP burden also
matters.
...
\textbf{False negatives ($\sim$7--8.5\% of malignant):} literature
identifies amelanotic melanomas, small lesions ($<$6\,mm), and early
superficial spreading melanomas as persistent DL failure
modes~\cite{b15}---all subtypes that lack the pigmented features CNNs rely
on. Our FN are consistent with these categories based on qualitative
inspection. \textbf{False positives ($\sim$1.4--2.8\% of benign):}
dysplastic naevi, atypical Spitz naevi, and irregular seborrhoeic keratoses
are the most commonly confused benign subtypes in dermoscopy~\cite{b5}.
Exp.~1 detects 951/1021 melanomas (FN~rate 6.86\%), acceptable

...
specificity~98.62\%). Fig.~\ref{fig:ablation} confirms the recall advantage of the full fusion model
from the earliest epochs, with Exp.~1 maintaining a consistent lead over the
image-only ablation throughout all 20 epochs.
...
\textbf{Exp.~1} (cosine annealing): the LR decays to near-zero by epoch~20,
letting the model settle into a sharper minimum---FN drops $80{\rightarrow}70$
with \emph{zero} extra FP, a ``free lunch.''
\textbf{Exp.~2} (AdamW): weight decay broadens the decision boundary, peaking
recall at 93.54\% but surging FP from 145 to 181.
\textbf{Exp.~3} ($\gamma{=}1.5$): less aggressive focal suppression restores
specificity---FP falls to 144 (lowest of fusion models) while FN rises
modestly to 74.

Table~\ref{tab:deltas} isolates the individual contribution of each change
by reporting deltas relative to the immediately preceding configuration.

\begin{table}[t]
\caption{Isolated contribution of each design change. $\Delta$ values are
         relative to the prior configuration. Recall and F1 from
...
\toprule
\textbf{Change} & \textbf{vs.} & $\boldsymbol{\Delta}$\textbf{F1} & $\boldsymbol{\Delta}$\textbf{Recall} & $\boldsymbol{\Delta}$\textbf{FN} & $\boldsymbol{\Delta}$\textbf{FP} \\
\midrule
Cosine sched.   & Baseline  & $+$0.0052 & $+$0.98\,pp & $-$10 & $\phantom{+}$0 \\
AdamW + WD      & Exp.~1    & $-$0.0129 & $+$0.40\,pp & $-$4  & $+$36 \\
$\gamma$1.5     & Exp.~2    & $+$0.0113 & $-$0.79\,pp & $+$8  & $-$37 \\
Metadata fusion & Exp.~4    & $-$0.0068 & $+$1.86\,pp & $-$19 & $+$39 \\
TTA (6 xforms)  & Baseline  & $+$0.0202 & $-$0.68\,pp & $+$7  & $-$55 \\
\bottomrule
\end{tabular}
\end{table}
...
analysis (Table~\ref{tab:hp}) over AUC alone~\cite{b16}.
...
flagged. Without this gate, a non-dermoscopic image (e.g.\ a car) can
yield high sigmoid confidence: sigmoid heads trained only on dermoscopic
images have no explicit abstain mechanism, so out-of-distribution inputs
that activate similar low-level CNN filters (edges, colour gradients) can
align with malignant-class weights and produce misleadingly high logits.
The Mahalanobis check operates on the 1280-d embedding rather than the
output logit, intercepting these cases by detecting that the feature
vector lies outside the dermoscopic distribution. Exp.~1--3 produce
tighter distributions, suggesting cosine annealing yields more compact
